    ...
    <div>
      <Card theme={theme} style={{ marginBottom: 20, background: "#1e1b4b", borderColor: "#22c55e" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#22c55e" }}>🔒 ĐÁNH GIÁ ẨN DANH</div>
            <div style={{ fontSize: 12, color: styles.textMuted }}>Sau khi đánh giá, không ai biết ai đã đánh giá ai.</div>
          </div>
        </div>
      </Card>
      ...
      {reviewer && !hasCompleted && (
        <>
          <Card theme={theme} style={{ marginBottom: 20 }}>
            ...
            <div style={{ fontSize: 13, color: styles.textMuted }}>
              🔒 Đánh giá của bạn sẽ được ẩn danh hoàn toàn.
            </div>
          </Card>
...
export default function App() {
  ...
  // Màn hình tạo nhóm mới (khi chưa có roomId và chưa có nhóm)
  if (!roomId && !hasGroup) {
    return (
  ...
  // Giao diện chính (khi đã có nhóm)
  return (
