...
function CongViec({ members, tasks, setTasks, theme, leader, currentReviewer }: any) {
  ...
  useEffect(() => {
    ...
    if (updated) { setTasks(updatedTasks); alert("⏰ Hệ thống đã tự động phân chia ngẫu nhiên các công việc tồn đọng quá 24h!"); }
  ...
  const addTask = () => {
    ...
    const newTask = { id: uid(), name: form.name, description: form.description, subtasks: form.subtasks.map(n => ({ id: uid(), name: n, assignee: null, status: "pending" })), deadline: form.deadline, complexity: form.complexity, status: "todo", productLink: "", submittedBy: "", createdAt: new Date().toISOString() };
  ...
  const nhanTaskCon = (taskId: string, subtaskId: string) => {
    ...
    setTasks((prev: any[]) => prev.map(t => {
      ...
      return { ...t, subtasks: t.subtasks.map((s: any) => s.id === subtaskId ? { ...s, assignee: currentReviewer, status: "accepted" } : s) };
    }));
  };
  ...
  const doiTrangThai = (id: string) => {
    setTasks((prev: any[]) => prev.map(t => {
      if (t.id !== id) return t;
      if (t.subtasks.some((s: any) => s.assignee === null) && t.status === "todo") { alert("⚠️ Không thể chuyển trạng thái khi vẫn còn việc phụ chưa có ai nhận!"); return t; }
    ...
    <div>
      ...
      {showForm && (
        <TheCard theme={theme} style={{ marginBottom: 16 }}>
          ...
          <div style={{ marginTop: 10 }}>{form.subtasks.map((st, i) => <div key={i} style={{ fontSize: 13, color: styles.textMuted }}>• {st}</div>)}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        {(tasks || []).map((t: any) => (
          <div key={t.id} style={{ background: styles.cardBg, border: `1px solid ${styles.border}`, padding: 18, borderRadius: 14 }}>
            ...
            <div style={{ margin: "10px 0" }}>
              {t.subtasks.map((s: any) => (
                <div key={s.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 13, padding: "4px 0" }}>
                  <span style={{ color: s.assignee ? styles.text : "#f59e0b" }}>{s.assignee ? "✅" : "⬜"} {s.name} {s.assignee && `(${members.find((m: any) => m.id === s.assignee)?.name})`}</span>
                  {!s.assignee && <button onClick={() => nhanTaskCon(t.id, s.id)} style={{ background: "none", border: "none", color: "#6366f1", cursor: "pointer" }}>Nhận việc</button>}
                </div>
              ))}
            </div>
            <button onClick={() => doiTrangThai(t.id)} style={{ width: "100%", padding: 10, background: TRANG_THAI[t.status as "todo"].color + "22", color: TRANG_THAI[t.status as "todo"].color, border: "none", borderRadius: 8, fontWeight: 700, cursor: "pointer" }}>
              {TRANG_THAI[t.status as "todo"].label} → Nhấn đổi trạng thái
            </button>
          </div>
        ))}
      </div>
